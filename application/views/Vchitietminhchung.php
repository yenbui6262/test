...
<div class="container-fluid">
    <div class="panel panel-default">
            <div class="panel-heading text-left">
                <h4 style="color: #fff;" class="text-center m-0">{if !empty($params['tenct'])}{$params['tenct']}{/if}</h4>
            </div>
            ...
            <form action="{$url}quanlyminhchung" method="POST" class="insert" id="myForm">
                <div class="row">
                    ...
                    <div class="col-md-4 form-group" {if empty($params['thongtinsv'])}style="display:none"{/if}>
                        ...
                        <input type="text" id="tenct" class="form-control" aria-label="Small" placeholder="Nhập nội dung">
                    </div>
                    <div class="col-md-4 form-group" {if empty($params['thongtinsv'])}style="display:none"{/if}>
                        <label for="thoigianbd">Thời gian bắt đầu:</label>
                        <input type="date" id="thoigianbd" name="thoigianbd" value="{if !empty($thoigianbd)}{$thoigianbd}{/if}" class="form-control" aria-label="Small" placeholder="Nhập nội dung">
                    </div>
                    <div class="col-md-4 form-group" {if empty($params['thongtinsv'])}style="display:none"{/if}>
                        <label for="thoigiankt">Thời gian kết thúc:</label>
                        <input type="date" id="thoigiankt" name="thoigiankt" value="{if !empty($thoigiankt)}{$thoigiankt}{/if}" class="form-control" aria-label="Small" placeholder="Nhập nội dung">
                    </div>
                        ...
                        {if !empty($params['minhchung'])}
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="{$val.tLink}" class="btn btn-info"><i class="fas fa-eye" title="Link minh chứng"></i></a>
                                </td>
                            </tr>
                            {/foreach}
                        {else}
